...
describe('BST()', () => {
  ...
  describe('toArrDFSInOrder() :: depth-first, in-order', () => {
    test('empty BST', () => {
      var bst = new BST();
      expect(bst.toArrDFSInOrder()).toEqual([]);
    });

    test('BST with single root node', () => {
      var bst = new BST().insert(100);

      expect(bst.toArrDFSInOrder()).toEqual([100]);
    });
    ...
    test('BST with only lefts', () => {
      ...
      expect(bst.toArrDFSInOrder()).toEqual([30, 70, 100]);
    });
    ...
    test('BST with only rights', () => {
      ...
      expect(bst.toArrDFSInOrder()).toEqual([100, 150, 190]);
    });
    ...
    test('BST with lefts and rights', () => {
      ...
      var res = bst.toArrDFSInOrder();

      expect(res).toEqual([30, 50, 70, 100, 130, 150, 180, 190]);
    });
  });
});
